...
namespace ProjectDefense.Web.Services
{
    ...
    {
        public byte[] ExportToTxt(List<Rezerwacja> rezerwacje, string nazwaSali)
        {
            ...
            foreach (var rez in rezerwacje.OrderBy(r => r.CzasRozpoczecia))
            {
                var studentInfo = rez.Student != null 
                    ? $"{rez.Student.FirstName} {rez.Student.LastName}"
                    : "WOLNY";
                
                sb.AppendLine($"Data: {rez.CzasRozpoczecia:dd.MM.yyyy}");
                sb.AppendLine($"Godziny: {rez.CzasRozpoczecia:HH:mm} - {rez.CzasZakonczenia:HH:mm}");
        ...
        public byte[] ExportToExcel(List<Rezerwacja> rezerwacje, string nazwaSali)
        {
            ...
            foreach (var rez in rezerwacje.OrderBy(r => r.CzasRozpoczecia))
            {
                worksheet.Cell(currentRow, 1).Value = rez.CzasRozpoczecia.ToString("dd.MM.yyyy");
                worksheet.Cell(currentRow, 2).Value = rez.CzasRozpoczecia.ToString("HH:mm");
                worksheet.Cell(currentRow, 3).Value = rez.CzasZakonczenia.ToString("HH:mm");
        ...
        public byte[] ExportToPdf(List<Rezerwacja> rezerwacje, string nazwaSali)
        {
                ...
                container.Page(page =>
                {
                    ...
                    page.Content()
                        ...
                        .Table(table =>
                        {
                            ...
                            foreach (var rez in rezerwacje.OrderBy(r => r.CzasRozpoczecia))
                            {
                                table.Cell().Element(CellStyle).Text(rez.CzasRozpoczecia.ToString("dd.MM.yyyy"));
                                table.Cell().Element(CellStyle).Text(rez.CzasRozpoczecia.ToString("HH:mm"));
                                table.Cell().Element(CellStyle).Text(rez.CzasZakonczenia.ToString("HH:mm"));
